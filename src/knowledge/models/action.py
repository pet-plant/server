"""``probe_action`` — the remedial actions attached to a probe.

When a probe fires ("water deficit, visible as leaf droop"), these are the
...
class ProbeAction(Base):
    __tablename__ = "probe_action"
    ...
    probe: Mapped["Probe"] = relationship(back_populates="actions")

    def __repr__(self) -> str:  # pragma: no cover - debug aid
        return f"<ProbeAction {self.instruction[:32]!r}>"
